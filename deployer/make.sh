...
log_info "Building and tagging the deployer image as ${DEPLOYER_IMAGE_TAGGED}"
assert_exec_cmd docker build -t ${DEPLOYER_IMAGE_TAGGED} --build-arg REGISTRY=${REGISTRY} --build-arg TAG=${TAG} .
log_info "Pushing ${DEPLOYER_IMAGE_TAGGED}"
assert_exec_cmd docker push ${DEPLOYER_IMAGE_TAGGED}

log_info "Tagging and pushing ${IMAGE_CLONE_TAGGED}"
